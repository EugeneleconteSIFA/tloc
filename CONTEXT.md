...
- `demo-personnages.html` : trois personnages (paysanne, paysan, rôdeuse) au repos, en marche et en
  ...
  en compte beaucoup.
- **Manque pour aller plus loin : des animations.** Un seul clip est livré avec les packs — comblé en v26 par la Universal Animation Library 2 (voir plus bas).

## v26 — Animations reciblées et bestiaire (septembre 2026)

### Banque d'animations (Quaternius Universal Animation Library 2, CC0)
- **Le rig est identique** à celui des packs de personnages : 65 os, mêmes noms, même ordre, bind
  pose en T. Vérifié en comparant les listes d'articulations et les poses de repos. Seules les
  proportions diffèrent (bras 15 % plus longs, bassin 1,5 cm plus bas, quelques degrés sur la
  colonne) — pas de retarget lourd, juste un recalage.
- `prune_anims.py` (hors dépôt, dans le home) fabrique `assets_back/02_personnages/animations/ual2.glb` :
  maillages, peaux, matériaux et images supprimés, pistes de translation gardées pour `root` et
  `pelvis` seulement, pistes d'échelle supprimées. **8,09 Mo → 1,50 Mo, 42 clips.**
- **Piège qui a coûté un aller-retour** : on ne peut PAS jeter une piste constante. Dans un idle,
  les bras croisés sont tenus — donc constants — et les supprimer renvoie l'os à sa pose de repos,
  c'est-à-dire en croix. Une piste constante n'est supprimable que si sa valeur EST celle du repos.
  (C'est aussi ce qui fait disparaître `A_TPose` du lot : toutes ses pistes valent le repos.)
- `assets.js` : `animations()` charge la banque, `recibler(banque, clip, perso)` réécrit chaque
  piste en **delta sur la pose de repos** — `q_perso = repos_perso · repos_banque⁻¹ · q_clip` — et
  met les translations à l'échelle du personnage. `animer(perso, noms)` renvoie
  `{ mixer, actions, jouer(nom, fondu, boucle), update(dt) }`.
- `demo-animations.html` : un personnage complet, les 42 clips au menu déroulant.
- **Manque : la locomotion de base.** La volume 2 n'a ni Idle, ni Walk, ni Run, ni Jump simples
  (elle a du combat à l'épée, des idles typés, du zombie, de la ferme). Il faut la **volume 1**
  (« Universal Animation Library », Quaternius, CC0) pour Camille et les villageois.

### Matières du bestiaire
- Aucune créature du jeu n'a d'équivalent dans un pack (vérifié : rien de vivant dans nature,
  retro, megakit, props). Elles restent donc maison — règle 1 du BRIEF — et montent par leurs
  matières : `furTextures`, `featherTextures`, `membraneTextures`, `shellTextures`,
  `bareSkinTextures`, `hideTextures`, plus `grayMapFrom` et `creatureMat`. Chacune produit
  albédo + normales + rugosité, tuile raccordée aux bords (`bouclee`).
- Fabriquées **à la demande** via le proxy `CT` : ~30 ms de canvas chacune, et une partie du
  bestiaire ne sort jamais des galeries.
- `reliefPeau()` dans `geants.js` greffe normales et rugosité sur l'albédo d'origine d'un modèle
  de pack. **Il faut régler `repeat`** : l'atlas occupe tout l'UV 0..1, donc sans répétition une
  écaille de 5 mm est étirée sur 2 m de peau et ne se voit plus.

### Anatomie et animation des bêtes
- Rat refait : deux masses (croupe/tronc), tête séparée avec museau, oreilles en coupe, vibrisses,
  queue en **chaîne de 6 segments** qui ondule en fouet retardé, pattes avant et arrière distinctes.
- Corbeau : ailes en profil découpé (bord d'attaque droit, bord de fuite courbe) avec rémiges
  primaires séparées et queue en éventail, au lieu de planches. Pattes repliées sous le corps.
  **Attention au sens** : `ShapeGeometry` + `rotateX(-PI/2)` envoie +Y du dessin sur −Z du monde,
  donc le bord de fuite se dessine en +Y.
- Battements d'ailes : plus de sinus. Descente rapide et ample, remontée lente et repliée
  (`sin^0.55` / `−sin^1.5`), et la main de l'aile retarde de 0,7 rad sur l'épaule.
- `animeCreature(e, m, dt, speed)` est **exportée** : `demo-bestiaire.html` rejoue exactement la
  fonction du jeu, un banc d'essai qui ment ne sert à rien.

### Géants riggés (`geants.js`)
- Lydéric et Phinaert sont les seuls humanoïdes du bestiaire : ils passent sur le pipeline des
  personnages. `buildGeant('phinaert'|'lyderic')` renvoie `null` si la banque manque, et
  `game.js` retombe alors sur `makeGiant()` procédural — le jeu ne casse jamais faute d'un fichier.
- Clips : Phinaert en `Zombie_Idle_Loop` / `Zombie_Walk_Fwd_Loop` / `Sword_Heavy_Combo` /
  `Melee_Hook` / `Hit_Knockback` ; Lydéric en `Idle_FoldArms_Loop` / `Walk_Carry_Loop`.
- `E.setMaker(kind, fn)` et `E.setAnimHook(kind, fn)` permettent au niveau de remplacer un
  constructeur et son animation sans toucher au moteur.
- **La boîte englobante d'un maillage skinné ne tient pas compte du skinning** : `spawn({height})`
  donnait 43 m pour Phinaert et 630 m pour Lydéric. On mesure l'os `Head` (exact) et on ajoute
  ~12 cm de calotte crânienne.
- `carrure()` épaissit en redimensionnant les os. **L'axe d'un os est son +Y local** (mesuré : la
  translation locale de chaque enfant est purement en Y) : épaissir se fait donc en X/Z, un Y ≠ 1
  allonge ou raccourcit le membre.
- `socket()` accroche cornes, gourdin, heaume… en suivant **la position et l'orientation** de l'os,
  **jamais son échelle** : un objet parenté à la main hériterait du gonflement de `carrure()` et,
  les échelles étant anisotropes le long de la chaîne, serait aussi cisaillé. Option `suitRot=false`
  pour les pièces qui ne doivent prendre que la position — le bassin UE5 porte une rotation de repos
  de ~104° en X (convention Z-haut d'Unreal), une ceinture qui la suivrait partirait de travers.
- `demo-bestiaire.html` : les 8 créatures sur une rangée, cadrage par boîte englobante, case
  « en mouvement », `window.BANC.cadre(nom, facteur, lacet, tangage)` pour les captures.

### Pièges rencontrés
- **`?v=21` sur `engine.js`** : les modules du jeu importent `./engine.js?v=21`. Un nouveau module
  qui importe `./engine.js` charge une **deuxième instance** du moteur — deuxième scène, deuxième
  `window.TLOC`, menu vide. `bump.py` couvre désormais `geants.js` et `demo-bestiaire.html`.
- `window.TLOC` expose en plus `menu, newGame, resumeGame, spawnEnemy, KINDS, MAKERS,
  animeCreature, setMaker, setAnimHook` — de quoi lancer une partie et inspecter le bestiaire
  depuis la console.

## v27 — Camille et les habitants riggés (septembre 2026)

### Locomotion fabriquée (`locomotion.js`)
- La UAL2 ne livre **aucune locomotion de base**. Plutôt que d'attendre la volume 1, marche,
  course, repos, garde, arc, saut et les quatre poses de cinématique sont **fabriqués par code**
  sous forme de vrais `AnimationClip` : même mixer que les clips Quaternius, donc `crossFade`
  sans cas particulier. Quand la volume 1 arrivera, il suffira de retirer les noms de `CYCLES`.
- **L'axe d'un os est son +Y local** (mesuré : la translation locale de chaque enfant est
  purement en Y). Depuis la pose en croix, **Z** descend le bras le long du corps (≈1,26 rad),
  **X** donne le balancement et la foulée.
- **Flexion des doigts = X**, pas Z. Z ne fait qu'écarter : le premier essai donnait des mains
  en éventail sur tous les personnages. Une main légèrement refermée au repos change beaucoup.
- Le pied doit **compenser** cuisse + genou pour rester parallèle au sol (borné à ±0,45 rad),
  sinon la jambe avance pointe en bas et le personnage marche comme une danseuse.
- Le bassin monte **deux fois par cycle** (une fois par appui) : piste `pelvis.position`, le long
  de la verticale exprimée dans le repère du parent du bassin — le mannequin vient d'Unreal
  (Z vers le haut), un bond écrit naïvement en Y partirait de côté.

### Habitants et Camille (`pnj.js`)
- Six villageois, chacun avec tenue, teint, coiffure, sourcils, barbe et **idle différents** :
  c'est à eux qu'on parle, il faut les distinguer de loin. **142 000 triangles et 66 appels**
  pour les six.
- **Teinter se fait par MAILLAGE, pas par matériau** : tout le pack partage un seul matériau
  d'atlas (`MI_Peasant`), donc teinter par nom de matériau repeint la tenue entière d'un bloc.
  Les maillages, eux, s'appellent `..._Body`, `..._Legs`, `..._Arms_1`, `..._Feet`.
- **`rebind()` REPARENTE les maillages hors du groupe source** : récupérer les matériaux
  AVANT l'appel, sinon on teint un groupe devenu vide (les coiffures restaient grises).
- **Multiplier une couleur ne peut pas changer une teinte** : la tunique de rôdeuse est peinte
  en vert, aucune multiplication ne la rend bleue. `recolorer()` refait la texture au canvas avec
  les modes de fusion `saturation` puis `hue`, ce qui garde l'ombrage peint du pack et remplace
  la teinte. C'est ce qui rend son bleu à Camille.
- `prepare()` charge **chaque asset séparément** : une coiffure absente ne doit pas priver le
  village de ses six habitants. Seuls corps et tenues de base sont bloquants.
- Les sockets gardent l'**échelle propre** de l'objet accroché (`base`), sinon l'arc de Camille
  prenait l'échelle du personnage et devenait un arc de triomphe.
- `E.setPlayerMesh()` et `E.setCamilleHook()` permettent à un niveau de remplacer Camille sans
  toucher au moteur ; `PNJ.installerCamille(E)` fait les deux et est appelé par game, cave,
  house et tavern. Sans la banque, la Camille en primitives reste en place.

### Accessoires accrochés aux os : passer par la matrice, pas par la décomposition
- `carrure()` gonfle les os. Un accessoire dont l'offset est calé sur un os de taille 1
  flotte à côté d'une main 22 % plus large, et une défense s'enfonce dans un crâne
  agrandi de 24 %. C'étaient les trois défauts du BRIEF-GEANTS, tous la même cause.
- Décomposer la matrice puis multiplier l'offset par l'échelle **ne suffit pas** : le long
  d'un bras qui tourne, les axes de l'échelle et ceux de l'offset ne coïncident plus et la
  position part de travers (échelle monde de la main mesurée : 15,4 / 9,1 / 13,3 — très
  anisotrope). Il faut traiter l'offset comme un POINT dans l'espace local de l'os et le
  transformer par `it.os.matrixWorld` **complète** : `_p.copy(it.p).applyMatrix4(it.os.matrixWorld)`.
- L'objet accroché, lui, reste isotrope : `obj.scale.setScalar(ech * base)`, jamais l'échelle
  de l'os, sinon il est cisaillé.
- Recette mesurable plutôt qu'à l'œil : distance du manche à l'os `middle_01_r`, rapportée à
  la largeur de main (`middle_01` à `thumb_01`). Empoigné = moins de 0,6 largeur de main.
  Relevé actuel : 0,26 pour une main de 0,97.

### Taille en pose de repos ≠ taille debout
- `H` dans `buildGeant()` est la taille en POSE DE REPOS. Phinaert passe sa vie en
  `Zombie_Idle_Loop`, qui le voûte de ~12 % : à H = 7,6 il lisait 6,7 en jeu, soit la
  taille de Lydéric. D'où `H = 8,65` pour qu'il fasse ses 7,6 debout.
- Et la mise à l'échelle vient APRÈS `carrure()`, qui raccourcit le cou de l'ogre de 28 %.

### Échelle : le monde n'est PAS en mètres
- La Camille d'origine, en primitives, mesure **2,85 unités** du sol au sommet du crâne.
  Hauteur des portes, des marches, rayons de collision, recul de caméra et taille des géants
  sont tous réglés là-dessus. Un personnage riggé posé à « 1,72 m » paraît minuscule à côté
  des maisons et des géants — c'est l'erreur du premier jet.
- `pnj.js` expose `H_CAMILLE = 2.85` et `enUnites(metres)`. Toute nouvelle taille passe par là.
- Repères actuels : Camille 2,85 · villageois 2,7 à 3,0 · Lydéric 8,8 · Phinaert 9,0.
  Le rapport géant/Camille est ≈ 3,1, celui qu'avaient les géants en primitives.
- Dans `geants.js`, la mise à l'échelle vient **APRÈS** `carrure()` : celle-ci raccourcit le cou
  de l'ogre de 28 %, donc mesurer avant faisait mentir H de plus d'une unité et Lydéric
  finissait plus grand que Phinaert.

### Ne jamais sortir tôt de `updatePlayer()`
- Le bloc d'animation de Camille est **au milieu** de `updatePlayer()` : le ramassage des objets
  et TOUTES les interactions (parler, coffres, portes, leviers, dormir) sont plus bas dans la
  même fonction. Un `return` dans la branche riggée les désactive toutes, sans erreur console —
  le symptôme observé était « je ne peux plus parler à Lydéric ». La branche riggée est donc un
  `if (rigge) { … }` suivi d'un `if (!rigge) { … }`, jamais un `return`.

### Deux pièges qui ont coûté cher
- **`?v=` sur engine.js** : `bump.py` est passé en v22 pendant le travail, `pnj.js` était resté
  en v21 → **deuxième instance du moteur**, deuxième scène, `window.TLOC` écrasé, menu vide, et
  aucune erreur visible. `bump.py` **déduit désormais** la liste des fichiers (tout ce qui
  contient « engine.js ») au lieu de la lire en dur : le piège ne peut plus se reproduire.
- **`const E = new THREE.Euler()` local dans game.js** masque le namespace `E` du moteur.
  `E.capsulesNear(...)` y valait `undefined` et le niveau ne démarrait plus. `capsulesNear` est
  maintenant importé par son nom. **Tout autre `E.quelqueChose` écrit dans une de ces fonctions
  tombera dans le même piège** — il y en a au moins cinq (lignes ~480, 593, 721, 828, 950).

### Vérifié en jeu
- `index.html`, `cave.html`, `house.html`, `tavern.html`, `mage.html` : chargement sans erreur,
  Camille riggée avec ses 7 accessoires accrochés, niveau construit.
- Citadelle : **6 villageois riggés et animés, 2 géants riggés et animés, Camille riggée**,
  zéro erreur console.
- Coût des six villageois sur le banc : 142 000 triangles, 66 appels de rendu.

### Bancs d'essai
- `demo-pnj.html` : les six habitants, idle / marche / parle.
- `demo-animations.html` : les 42 clips Quaternius **plus** la locomotion fabriquée
  (`A.nomsClips()`), avec `BANC.vue()` et `BANC.fige(u)` — un cycle de marche se juge de profil.
...
- Matériaux manquants (2e temps) : tuiles et ardoises de toit, enduit/plâtre, écorce (les pavés de rue sont faits, procéduralement).
- **À télécharger : Universal Animation Library volume 1** (Quaternius, CC0) — locomotion de base absente de la volume 2. En attendant, `locomotion.js` la fabrique (v27).
- Coût des personnages riggés : ~24 500 triangles pièce. Prévoir une simplification si le village s'agrandit.
